...
def test_get_all_happy(client):
    params = {
        "filter_dict": json.dumps({ 'profile_username' : "user2"})
    }
...
def test_get_all_sad(client):
    params = {
        "filtero_dict": json.dumps({ 'profile_username' : "user2"})
    }
    result = client.get("/post", query_string=params)
...
def test_post_happy(client):
    ...
    result = client.post("/post", data=json.dumps(post_dict), headers={"Authorization": "Bearer " + user1_token}, content_type="application/json")
    print(result.json)
...
def test_post_sad(client):
    ...
    result = client.post("/post", data=post_dict, headers={"Authorization": "Bearer " + user1_token})
    assert result.status_code == 500
